...
class Dados_Dispositivo {
    ...
    static processar_dados(leituras) {
        ...
        this.atualizar_grafico("grafico_temperatura", "Temperatura", temperatura.map(d => d.vez_lida.toString()), temperatura.map(d => Number(d.valor)), "grafico_temperatura");
        this.atualizar_grafico("grafico_humidade", "Humidade", humidade.map(d => d.vez_lida.toString()), humidade.map(d => Number(d.valor)), "grafico_humidade");
        this.atualizar_grafico("grafico_peso", "Peso", peso.map(d => d.vez_lida.toString()), peso.map(d => Number(d.valor)), "grafico_peso");
        this.atualizar_grafico("grafico_ruido", "Ruído", ruido.map(d => d.vez_lida.toString()), ruido.map(d => Number(d.valor)), "grafico_ruido");
    }
    static atualizar_grafico(canvas_id, titulo, labels, valores, propriedade) {
        ...
        if (!canvas)
            return;
        this[propriedade] = new auto_1.default(canvas, {
            type: "line",
            data: {
                labels,
                datasets: [{
